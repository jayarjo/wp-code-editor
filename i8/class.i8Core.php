...
class WCE_i8 {
    ...
    public $classname, $namespace = 'wce_i8_';
	
	private $_defaults = array(), $_options = array(), $_option_fields = array();
	
	public $i8_path, $i8_url, $path, $url, $upload_path, $upload_url;
	...
	function __construct()
	{
		...
		# setting i8 path
		if (defined("I8_PLUGIN_MODE")) {
			$plugin_dir = basename(dirname(__FILE__));
			$this->i8_url = WP_PLUGIN_URL . '/' . $plugin_dir;
			$this->i8_path = WP_PLUGIN_DIR . '/' . $plugin_dir;
		} else {
	...
	function hooks_register($obj = false)
	{
		...
		foreach ((array)$methods as $method) {
			$this->hook_register($method, false, $obj);
		}
	}
	
	
	function hook_register($method, $override = false, $obj = false)
	{			
		...
		switch ( $handle ) :
			case 'a':
			case 'action':
				add_action( $tag, array($obj, $method), $priority, $args );
				break;
			case 'f':
			case 'filter':
				add_filter( $tag, array($obj, $method), $priority, $args );
				break;
			case 'sc':
			case 'shortcode':
				add_shortcode( $tag, array($obj, $method) );
				break;
		endswitch;
	}
	...
	function _unauth_wp_ajax() // on admin_init
	{
		...
		{
			if (isset($_REQUEST['action']) && in_array($_REQUEST['action'], $this->wp_ajax_)) {
	...
	function route2($handle = false)
	{
		...
		$handle = str_replace($this->namespace, '', $handle);
		...
		if (!class_exists("{$this->prefix}Ctrl")) {
			$this->load("{$this->i8_path}/base.WPCtrl.php");
		}
		...
		if (!method_exists($this->ctrls[$ctrl], $action)) {	
			return null;
		}	
	...
	function _pages_add() // on admin_menu
	{
		...
		if (!empty($this->pages)) {
			foreach ($this->pages as $page) {
				if (preg_match('|options?(?:_form)?$|', $page['handle']) || isset($page['parent']) && $page['parent'] == 'options') {
		...
		for ( $i = 0, $max = sizeof($this->pages); $i < $max; $i++ ) :
			...
			if (strpos($p['handle'], '/') !== false) {
				$p['handle'] = $this->namespace . $p['handle'];
			} 
	...
	//	Notices Management
	private function _add_msg($params)
	{
		if (empty($params['message'])) {
			return;
		}

		$msgs = $this->get_sess_var('msgs');
		if (!$msgs) {
			$msgs = array();
		}

		$msgs[] = $params;
		$this->set_sess_var('msgs', $msgs);
	}
	...
	function warn($params)
	{
		...
		$this->_add_msg(wp_parse_args($params, $defaults));
	}
	...
	{
		if (!$msgs = $this->get_sess_var('msgs')) {
			return;
		}
	...
	{
		register_setting($this->options_handle, $this->options_handle, array(&$this, 'options_sanitize'));
	}
	...
	function options_sanitize($input)
	{
		...
		{
			if (isset($this->_option_fields[$name]))
			{
				$o = $this->_option_fields[$name];
				...
				elseif ('password' == $o['type'] && empty($input[$name])) {
					$input[$name] = $this->o($name);
				}
			}
		}

		return apply_filters("i8_options_sanitize_{$this->classname}", $input);
	}
	...
	function options($from_db = false)
	{
		...
		$this->_options = array_merge($this->_defaults,  !empty($options) ? $options : array());
		return $this->_options;
	}
	...
	protected function _parse_options($options = false, $section = false)
	{
		...
		foreach ($options as $id => $o) {
			...
			elseif (isset($o['type'])) // requires form field
			{
				...
				if ($section) {
					$this->_option_fields[$id] = $o; // for validation purposes we got to store field structure separately
				}
			} 
		}		
	}
	...
	{
		?><div id="section-<?php echo $name; ?>" class="<?php echo $this->prefix; ?>section <?php echo $o['class']; ?>">
			<h3><?php echo $o['label']; ?></h3><?php
			if (isset($o['desc'])) {
	...
	{
		extract($o);
		?><input id="option-<?php echo $name; ?>" type="text" name="<?php $this->the_o($name); ?>" class="<?php echo $class; ?>" value="<?php echo $this->o($name); ?>" /> <span class="description"><?php echo $desc; ?></span><?php
	}
	
	function options_field_textarea($name, &$o)
	{
		extract($o);
		?><textarea id="option-<?php echo $name; ?>" name="<?php $this->the_o($name); ?>" class="<?php echo $class; ?>"><?php echo $this->o($name); ?></textarea><br /> <span class="description"><?php echo $desc; ?></span><?php
	}
	
	function options_field_password($name, &$o)
	{
		extract($o);
		?><input id="option-<?php echo $name; ?>" type="password" name="<?php $this->the_o($name); ?>" class="<?php echo $class; ?>" value="" /> <span class="description"><?php echo $desc; ?></span><?php
	}
	...
	{		
		extract($o);
		?><input id="option-<?php echo $name; ?>" type="checkbox" name="<?php $this->the_o($name); ?>" value="1" <?php if ($this->o($name)) echo 'checked="checked"'; ?> /> <span class="description"><?php echo $desc; ?></span><?php
	}
	...
	function options_field_select($name, &$o)
	{
		...
		?><select id="option-<?php echo $name; ?>" name="<?php $this->the_o($name); ?>" class="<?php echo $class; ?>">
	...
	function options_field_radiogroup($name, &$o)
	{
		?><div id="option-<?php echo $name; ?>">
		<?php foreach ($o['items'] as $value => $label) : ?>
        <label>
            <input type="radio" name="<?php $this->the_o($name); ?>" value="<?php echo $value; ?>" <?php 
                if ($value == $this->o($name)) echo 'checked="checked"'; 
            ?> />
	...
	/* Helpers */
	function load($path, $vars = null, $buffer = false)
	{
		if (file_exists($path))
		{
			if (!is_null($vars))
	 ...
	 * @return string
	 */
	function get_page_template($id = false) 
	{
		if (!$id) {	
			$id = get_queried_object_id();
			$pagename = get_query_var('pagename');
		} elseif (is_string($id)) {
			$post = self::get_post_by_slug($id);
			$pagename = $id;
			$id = $post->ID;
		} else {
			$post = get_post($id);
		...
		$template_path = get_query_template( 'page', $templates );
		return !empty($template_path) ? $template_path : false;
	}
	...
	function get_post_by_slug($slug, $posts = false)
	{
		...
		global $wpdb;
		return $wpdb->get_row("SELECT * FROM $wpdb->posts WHERE post_name = '$slug'");
	}
	...
	function is_addedit_page($post_type)
	{
		...
		return (is_admin() && $typenow == $post_type && in_array($GLOBALS['pagenow'], array('post-new.php', 'post.php', 'edit.php')));
	}
	...
	// session management
	private function &_get_sess_storage()
	{
		$name = "{$this->namespace}sess";
	...
	function set_sess_var($name, $value, $one_time = false)
	{
		$storage =& $this->_get_sess_storage();
		$storage[$name] = compact('value', 'one_time');
	}

	
	function get_sess_var($name)
	{
		$storage =& $this->_get_sess_storage();
		if (isset($storage[$name])) {
			$value = $storage[$name]['value'];
			if ($storage[$name]['one_time']) {
				unset($storage[$name]);
			}
			return $value;
		}
